...
// The internal inode structure for all files, directories, and links in
// a file system namespace.
class Stat {
  char file_ino_[8];
  char file_size_[8];      // Not overloaded to represent directory sizes
  char file_mode_[4];      // File type and access modes
  char zeroth_server_[4];  // Only makes sense for directories
  char user_id_[4];
  char group_id_[4];
...
// and reuse the result until the specified due.
class DirEntry {
  char dir_ino_[8];
  char dir_mode_[4];
