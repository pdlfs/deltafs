...
TEST(DirIndexTest, Empty) {
  ASSERT_EQ(idx_->FetchBitmapRadix(), 0);
  ASSERT_EQ(idx_->FetchDirId(), dir_id_);
  ASSERT_EQ(idx_->FetchZerothServer(), zeroth_server_);
...
TEST(DirIndexTest, ScaleUp1) {
  ...
  for (int r = 1; r <= kNumRadix; r++) {
    ...
    ASSERT_EQ(idx_->FetchBitmapRadix(), r);
  }
}
...
TEST(DirIndexTest, Recover1) {
  ...
  ASSERT_EQ(recovered->FetchBitmapRadix(), 3);
  ASSERT_EQ(recovered->FetchDirId(), dir_id_);
  ASSERT_EQ(recovered->FetchZerothServer(), zeroth_server_);
  delete recovered;
}
...
TEST(DirIndexTest, Recover2) {
  ...
  ASSERT_EQ(recovered->FetchBitmapRadix(), 9);
  ASSERT_EQ(recovered->FetchDirId(), dir_id_);
  ASSERT_EQ(recovered->FetchZerothServer(), zeroth_server_);
  delete recovered;
}
...
TEST(DirIndexTest, Hash) {
  ...
  ASSERT_TRUE(DirIndex::GetNameHash("", hash0) == 8);
  ASSERT_TRUE(DirIndex::GetNameHash(File(1), hash1) == 8);
  ASSERT_TRUE(DirIndex::GetNameHash(File(2), hash2) == 8);
  ...
  for (int i = 0; i < 1000000; i++) {
    DirIndex::GetNameHash(File(i), hash);
